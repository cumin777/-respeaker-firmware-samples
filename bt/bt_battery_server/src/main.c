...
static uint8_t app_button_cb_battery(void)
{
	battery_level -= 1;
	if (battery_level == 0) {
		battery_level = 100;
...
static struct my_bas_cb bas_app_callbacks = {
	.button_cb = app_button_cb_battery,
};
...
int main(void)
{
	...
	err = my_bas_init(&bas_app_callbacks);
	if (err) {
		printk("Failed to init BAS (err:%d)\n", err);
		return -1;
	}
